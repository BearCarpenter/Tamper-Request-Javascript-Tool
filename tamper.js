var AjaxMonitoringSample = {
  ...
  resetAllowedRequests() {
    this.allowed_requests = [];
  },
	...
	createDiv(){
		...
		<div>
			...
			<input type="text" value="GET" id="`+this.mainDivId+`_method" style="width:100%;" placeholder="Method" /> 
  ...
  submitClicked() {
    ...
    var params = JSON.stringify(JSON.parse(document.getElementById(this.mainDivId + "_params").value || null));
    var url = document.getElementById(this.mainDivId + "_url").value;
    var method = document.getElementById(this.mainDivId + "_method").value;

    var http = new XMLHttpRequest();
    http.open(method, url, true);

    var headers = JSON.parse(document.getElementById(this.mainDivId + "_headers").value || null);
    ...
    http.onreadystatechange = function () {
      if (http.readyState == 4 && http.status != 999) {
        try {
          debugger;
          const body = http.response !== '' ? JSON.parse(http.response) : null;
          debugger;
        } catch (error) {
  ...
  toggleMonitoring(enable_disable) {
    ...
    this.statusColorize();
    if (this.monitoring_status) this.allowedByAutoDisabling = true;
  },
